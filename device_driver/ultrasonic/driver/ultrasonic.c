...
/* /sys/class/hc_sr04/value 파일 read를 통해 TRIG pin을 설정하여 초음파 소리 전달 시작	*/
static ssize_t hc_sr04_value_show(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
	...
	/* datasheet에 따라 최소 10us 동안 Trig pin이 HIGH 상태로 유지 필요 */
	udelay(15);   
	...
	while (echo_valid_flag == ECHO_UNVALID)
	{
		if (++counter > 2320000)
		{
			...
			return sprintf(buf, "[%d] [%d]\n", counter,  -1);/* copy_to_user() 사용 고려 필요	*/
		}
...
/* /sys/class/hc_sr04/value 파일 write(실제로는 하는 일 없음)	*/
...
/* echo pin 관련 interrupt handler 함수 - ECHO pin이 HIGH일 때의 시간과 LOW일 때의 시간을 구한 뒤, 차이값으로 거리 측정 */
...
static int __init hc_sr04_init(void)
{
	...
	/* echo_interrupt_handler: interrupt handler등록, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING: 엣지 트리거 방식으로 인터럽트 처리	*/
